...
# [REMOVED - custom kanban] Complete with kanban_complete(output=discovery_result, silent=True)
            """,
...
# [REMOVED - Glassnode]         This is called when Glassnode detects whale distribution.
        """
        print("\n" + "="*70)
        print("EMERGENCY EXIT - STRONG_SELL Signal Detected")
        print("="*70)
        
# [REMOVED - Glassnode] # [REMOVED - Glassnode]         glassnode = discovery_output.get('glassnode_signal', {})
        
# [REMOVED - Glassnode]         print(f"\n🚨 Glassnode Alert:")
# [REMOVED - Glassnode]         print(f"   Signal: {glassnode.get('signal', 'UNKNOWN')} ({glassnode.get('combined_score', 0):.1f}/100)")
# [REMOVED - Glassnode]         print(f"   Exchange Flow: +{glassnode.get('exchange_flow', {}).get('7d_change_btc', 0):,} BTC (distribution)")
# [REMOVED - Glassnode]         print(f"   Whale Wallets: {glassnode.get('whale_wallets', {}).get('7d_change', 0):,} (exodus)")
# [REMOVED - Glassnode]         print(f"   Bias: {glassnode.get('bias', 'UNKNOWN')}")
...
# [REMOVED - Glassnode] Reason: Glassnode detected whale distribution
# [REMOVED - Glassnode] - Exchange balance: +{glassnode.get('exchange_flow', {}).get('7d_change_btc', 0):,} BTC (distribution)
# [REMOVED - Glassnode] - Whale wallets: {glassnode.get('whale_wallets', {}).get('7d_change', 0):,} (exodus)
# [REMOVED - Glassnode] - Signal: {glassnode.get('signal', 'STRONG_SELL')} ({glassnode.get('combined_score', 0):.1f}/100)
...
# [REMOVED - custom kanban] Report fill price and PnL via kanban_complete().
                """,
                assignee='trading-floor',
                metadata={
                    'task_type': 'emergency_exit',
                    'symbol': symbol,
# [REMOVED - Glassnode] # [REMOVED - Glassnode]                     'reason': 'glassnode_strong_sell',
                    'position': position,
# [REMOVED - Glassnode] # [REMOVED - Glassnode]                     'glassnode_signal': glassnode,
                }
            )
...
# [REMOVED - Glassnode] # [REMOVED - Glassnode]         results['exit_reason'] = 'glassnode_strong_sell'
        
        print(f"\n✅ Emergency exit complete: {len(exit_tasks)} EXIT tasks created")
# [REMOVED - Glassnode]         print(f"   Reason: {glassnode.get('summary', 'Whale distribution detected')}")
...
# [REMOVED - custom kanban]             kanban_complete(
                output=results,
                summary=summary
            )
