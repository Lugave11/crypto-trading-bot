...
# Bot Settings v4 - PROFITABILITY OPTIMIZED
BOT_SETTINGS = {
    # RSI (core) - WIDER levels for crypto
    'rsi_oversold': 20,    # Was 25 - wait for deeper oversold
    'rsi_overbought': 80,  # Was 75 - wait for deeper overbought
    
    # TREND FILTER - 200 EMA
    'use_ema_filter': True,
    'ema_period': 200,
    
    # TREND STRENGTH - ADX (trade RANGING markets for mean reversion)
    'use_adx_filter': True,
    'adx_period': 14,
    'adx_max': 30,  # Was 25 - allow slightly stronger trends
    
    # VOLATILITY - ATR (WIDER stops to avoid noise)
    'use_atr_stops': True,
    'atr_period': 14,
    'atr_stop_mult': 2.5,      # Was 1.5 - wider initial stop
    'atr_trailing_mult': 3.5,  # Was 2.0 - wider trailing to let profits run
    
    # VOLUME CONFIRMATION - RELAXED
    'use_volume_confirm': False,  # DISABLED - was filtering out good entries
    'volume_period': 20,
    'volume_mult': 1.0,  # No minimum volume requirement
    
    # DIVERGENCE DETECTION - HIGHER WEIGHT
    'use_divergence': True,
    ...
    # TAKE PROFIT - NEW! Target profits instead of just trailing
    'take_profit': 1.5,      # 1.5% profit target (was 0.8%)
    'stop_loss': 2.0,        # 2.0% fallback stop (was 1.0%)
    'trailing_stop_pct': 1.5,
    'trailing_check_interval': 10,  # Was 5 - check less often to avoid noise
    
    # NEW: Profit target before trailing activates
    'profit_target_mult': 2.0,  # Activate trailing after 2x ATR profit
}
    ...
    def update(self, candle):
        ...
        try:
            ...
            if self.position is None:
                ...
                # EXIT LOGIC - TAKE PROFIT + TRAILING STOP
                exit_p = None
                exit_reason = None
                ...
                if self.position['type'] == 'LONG':
                    ...
                    profit_target = self.position['entry'] * (1 + self.settings['take_profit'] / 100)
                    
                    # TAKE PROFIT - NEW! Exit at profit target
                    if row['high'] >= profit_target and pnl_pct > 0:
                        exit_p = profit_target
                        exit_reason = f'Take Profit ({self.settings["take_profit"]}%)'
                    
                    # Trailing stop
                    elif row['low'] <= self.position['sl']:
                        exit_p = self.position['sl']
                        exit_reason = f'Trailing SL (ATR)' if self.settings['use_atr_stops'] else 'Trailing SL'
                    
                    # RSI mean reversion exit (only if small profit or small loss)
                    elif rsi_val and rsi_val > 65 and pnl_pct > -0.5:
                        exit_p = current_price
                        exit_reason = 'RSI Exit'
                
                else:  # SHORT
                    pnl_pct = (self.position['entry'] - current_price) / self.position['entry'] * 100
                    profit_target = self.position['entry'] * (1 - self.settings['take_profit'] / 100)
                    
                    # TAKE PROFIT - NEW! Exit at profit target
                    if row['low'] <= profit_target and pnl_pct > 0:
                        exit_p = profit_target
                        exit_reason = f'Take Profit ({self.settings["take_profit"]}%)'
                    
                    # Trailing stop
                    elif row['high'] >= self.position['sl']:
                        exit_p = self.position['sl']
                        exit_reason = f'Trailing SL (ATR)' if self.settings['use_atr_stops'] else 'Trailing SL'
                    
                    # RSI mean reversion exit (only if small profit or small loss)
                    elif rsi_val and rsi_val < 35 and pnl_pct > -0.5:
                        exit_p = current_price
                        exit_reason = 'RSI Exit'
...
# ============================================================================
log("="*70)
log("🚀 PAPER TRADING BOT V4 - PROFITABILITY OPTIMIZED")
log("="*70)
log(f"Pairs: {', '.join(PAIRS_CONFIG.keys())}")
log(f"Timeframe: {TIMEFRAME}")
log("")
log("📊 V4 IMPROVEMENTS:")
log(f"  ✅ Wider RSI levels (20/80) - deeper reversals")
log(f"  ✅ Wider ATR stops (2.5x/3.5x) - less noise")
log(f"  ✅ Take Profit target ({BOT_SETTINGS['take_profit']}%)")
log(f"  ✅ Relaxed volume filter (disabled)")
log(f"  ✅ Better RSI exits (65/35 threshold)")
log("")
