...
Managed by Orchestrator via tasks, stored in `state/coin_universe.json`.
...
├── [REMOVED]_bulk_fetch.py     # Load logic
...
```json
{
  ...
  "metadata": {
    "description": "Dynamic coin universe - managed by Orchestrator via tasks",
    "[REMOVED]_coverage": "100% (all coins have on-chain data)",
...
| `metadata.[REMOVED]_coverage` | `string` | Coverage status |
...
def load_coin_universe() -> List[str]:
    ...
    This file is managed by the Orchestrator via tasks.
...
```python
from [REMOVED]_bulk_fetch import load_coin_universe, fetch_[REMOVED]_bulk
...
def scan_all_candidates():
    ...
    # Step 2: Fetch [REMOVED] for ALL coins in universe
    [REMOVED]_data = fetch_[REMOVED]_bulk(coins)
    
    # Step 3: Process each coin
    for symbol in coins:
        signal = [REMOVED]_data.get(symbol, get_default_signal())
        
        # Build discovery report
        coin_data = {
            'symbol': symbol,
            '[REMOVED]_signal': signal,
...
[REMOVED](
    ...
    **Reason:** Volume spike $50M→$500M, [REMOVED] coverage added
    ...
    **New Universe:**
    ...
    metadata={
        ...
        'reason': 'Volume spike + [REMOVED] coverage',
    }
)
```
...
def validate_coin_universe(coins: List[str]) -> bool:
    ...
    # Rule 4: All coins must have [REMOVED] coverage
    for coin in coins:
        if not has_[REMOVED]_coverage(coin):
            print(f"❌ Validation failed: {coin} has no [REMOVED] coverage")
            return False
...
**Trigger:** PEPE volume spikes from $50M → $500M, [REMOVED] adds coverage
...
```json
{
  ...
  "updated_by": "t_abc123",
  "reason": "PEPE volume spike $50M→$500M, [REMOVED] coverage added"
}
```
...
| `reason` | Why | `"PEPE volume spike + [REMOVED] coverage"` |
...
| **Autonomy** | Manual edits | Orchestrator decides via Hermes |
...
| `[REMOVED]_bulk_fetch.py::load_coin_universe()` | Load logic | 38 |
| `[REMOVED]_bulk_fetch.py::fetch_[REMOVED]_bulk()` | Bulk fetch | ~100 |
| `COIN_UNIVERSE_LOGIC.md` | This doc | - |
...
### Load Coins (Python)
```python
from [REMOVED]_bulk_fetch import load_coin_universe
...
### Fetch [REMOVED]
```python
from [REMOVED]_bulk_fetch import fetch_[REMOVED]_bulk

coins = load_coin_universe()
data = fetch_[REMOVED]_bulk(coins)
