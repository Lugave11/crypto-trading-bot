...
Every 5 min: Cron → Data Collection Task (trading-data)
    ...
    ↓
    [REMOVED](output={coin_data, alerts, whale_scores})
...
Every 15 min: Cron → Orchestrator Task (trading-orchestrator)
    ...
    ↓
    [REMOVED](output={decisions, tasks_created})
...
Method Bot (trading-{method}):
    ...
    ↓
    [REMOVED](output={entry, size, status})
    ↓
...
- Every task reports via `[REMOVED](output=...)`
